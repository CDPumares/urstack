...
**Save look pack** writes a `.tar.xz` you can keep or move to another Fedora machine. **Browse themes** lists GitHub picks first (Dracula, Nordic, Sweet, Catppuccin, Candy icons, Kora, Bibata, Nordzy), then the [GNOME Look](https://www.gnome-look.org) / [KDE Look](https://www.kde-look.org) community catalog — the same openDesktop store Discover uses, including screenshots. **Install** downloads a free archive, copies it into your home directory, and switches this desktop to it. Third-party `install.sh` scripts inside a zip are not executed. Archives cannot write outside the extract dir and do not install into `/usr`.
